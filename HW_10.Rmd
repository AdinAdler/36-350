...
```{r,error=TRUE}
f = function(x) { 
  if ( typeof(x) != "character") warning("x should be numeric \n")
  tolower(x) 
}
f(10)
```
...
```{r,error=TRUE}
f = function(x) { 
  if ( typeof(x) != "character") stop("x should be numeric \n")
  tolower(x) 
}
f(10)
```
...
test_that(desc = "Test for strings with 0 overlap different length",   
          expect_equal(word.similarity("fooqwetyuiop","bar"),0))
...
test_that(desc = "Test for bad input",           
          expect_error(word.similarity(5,"")))
